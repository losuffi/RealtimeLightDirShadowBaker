...
float CalcAttenByShadowCube(samplerCUBE _ShadowCube, float _Dist , float3 worldPos , float3 CubePosition, float3 LightDir)
{
    float3 lastPosition = worldPos + _Dist * normalize(LightDir) ;
    float3 relativeDir = normalize(lastPosition - CubePosition);
    float relativeDepth = texCUBE(_ShadowCube, relativeDir).r;
    return step(0.9, relativeDepth);
}